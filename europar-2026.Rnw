...
<<europar.initial.base.model, echo=FALSE, fig.cap="Model of energy consumption for the baseline experiment.", fig.height=4>>=
library(dplyr)
...
We will try to adjust two different models to this data, one linear on the initial temperature and other that includes also a quadratic term; in both cases we will also include the problem dimension (a problem parameter) and the population size (an algorithm parameter).
