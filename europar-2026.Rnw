...
plot_predictions(europar_test_delta_evals_model, condition = c("initial_temp", "dimension","population_size"))
@

This model is plotted in Figure \ref{fig:europar.initial.model.slopes} for the three problem dimensions. What is interesting here is that the the slope is different for 3 and 5 dimensions, implying that depending on the problem size, a specific parameter combination will affect them in a different way. But the main issue here is that we have a wide range of temperatures and a very wide range of effects on energy consumption; since our initial intent is to reduce energy consumption, by design we should strive in designing experiments that, from this phase, would already reduce energy consumption, at the same time we gather heuristic information on how to optimize the algorithm implementation for energy production. Reducing the slope of dependence on temperature would be a way of reducing energy consumption, since the system temperature is mostly out of our control.

Since we cannot simply lower the temperature of the system, we need to try different approaches and measure how they influence this PKG vs. temperature model.
