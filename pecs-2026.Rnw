...
@
%
However, what we observe after processing to extract workload values is that variability is still quite high; the interquartile range for $\Delta E$ is \Sexpr{IQR_europar_test_processed_delta_PKG}. Again, we try to model this dependence using a generalized linear model, using a term quadratic on the temperature or not, and including population size, the dimension as factor and the number of evaluations, which should change every run as a result of the stochastic algorithm. Again, the model quadratic in the temperature that includes the number of evaluations is better with residual deviation \Sexpr{round(difference_delta_models$`Resid. Dev`[1],2)} compared to \Sexpr{round(difference_delta_models$`Resid. Dev`[2],2)}.
...
, we see that the models are essentially the same. The experimental set up does not change the model, it changes the parameters that affect the model; we need then evaluate what is the influence on the two main operational context variables we can observe, temperature and time.
%
