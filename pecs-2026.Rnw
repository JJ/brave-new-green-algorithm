...
The second challenge is even more determinant: the operational context of the system will influence the energy consumed, and it will do so not only in a way that affects the baseline measurements, but also the relative differences between the baseline and workload+baseline \cite{freina2024survey,cruz2025}.
% Removed the micro definition of operational context, this is described before in the abstract.
% If needed here, I think is better to introduce the concept before relating it to the challenge of measuring energy consumption. - M
% OK- JJ
There are several reasons for this, but they can be summarized as follows: the chipset and the operating system actively work to keep the system's temperature within a specified range. In order to do that, they actively manage power consumption, aiming to minimize it using different techniques such as dynamic voltage and frequency scaling (DVFS) or dynamic power management (DPM) \cite{snowdon2005power}.

Since, in general, scientists work on personal computers, it follows that the measure of energy we are interested in is the one taken in a real, non-dedicated system, where other processes will be running at the same time. In general, this means that the state of the system will be changing all the time: measures for temperature as well as power reduction will be active the whole duration of any experiment, but the most important thing is that they will be triggered not only by the workload, even if it is a substantial one, one that implicates several cores, but also by others; that is, there is no control over the {\em base} state of the system, however you want to define it, and no control over how this context will affect the workload that is being measured.
...
%% There are two 'This is..' are both sentences needed? it appears as if one is replacing the other. - M
%% Eliminada la primera frase - JJ
as captured by sensors, in the energy spent by a workload. We will test different experimental setups, checking for changes on the underlying model, as well as looking for one that better captures the variance in energy measurements caused by the independent or dependent variables in the algorithm.
...
Besides these two parameters, we will also be changing the number of generations without change needed to finish the algorithm. Its effect is too small to be measured in this context, and for the purposes of this paper, it means that we will be making two {\em sandwich} runs for every combination of problem and population size, one for every value of this parameter.
