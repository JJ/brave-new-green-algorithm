...
There are several reasons for this, but they can be summarized as follows: the chipset and the operating system actively work to keep the system's temperature within a specified range and to prevent it from exceeding a critical threshold. In addition to this task, over which the user has little or no control, modern systems actively manage power consumption, aiming to minimize it using different techniques such as dynamic voltage and frequency scaling (DVFS) or dynamic power management (DPM) \cite{snowdon2005power}.
...
This is precisely what we will carry out in this paper. This is why in this paper, we will run a set of experiments and analyze them, looking for a model of the influence of the CPU temperature,
