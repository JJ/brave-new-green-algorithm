...
Our interest in heuristic measurement of the influence of temperature on energy consumption arose from our line of research that tries to optimize it in metaheuristics; this is why we will be making measurements on a population-based metaheuristic called Brave New Algorithm \cite{merelo2022brave}. The implementation has been written on the language Julia, a JIT-compiled language with an interesting concurrent model. However, we will be using a single-threaded process and testing different configurations to minimize the Sphere function, varying the parameters: problem dimension (3, 5 or 10) and population size (200 or 400). Our initial intention was to compare different parameters, software and hardware configurations; however, for the purpose of this paper, the main point is that we will be running experiments every one of which takes several seconds to complete, with the whole experiment for all configurations taking about two hours. The duration is enough to make the system enter different states, and so that it theoretically has enough energy consumption differences between configurations to be detectable.
