...
Figure \ref{fig:evoapps.perf.boost} shows how the optimized mutation changes the energy profile of the implemented algorithm; in general, and as anticipated above, it consumes less energy for the same time, with black points representing it taking the lower half of the figure, in general. We still see that the dispersion of results is a problem: these new measurements still have energy differences over baseline which are close to 0, but do not reach the (false) negative values they did before. This is why we will need to compare for every parametrization, checking if the differences advice in favor of this change for these sizes.
...
<<evoapps.uniform.mutation, echo=FALSE, warnings=FALSE, message=FALSE, fig.height=3, fig.pos="h!tb", fig.cap="Energy vs fitness difference to the optimum for all experiments. It follows the usual conventions, with black-filled points corresponding to the new implementation, white for the previous one.">>=
...
ggplot(new_results, aes(x=delta_PKG,y=log_diff)) +
  geom_point(fill=new_results$fill,alpha=0.5,size=new_results$point_size,shape=new_results$shape) + theme(legend.position="none") + labs(x="Energy (J)", y="log(diff. target)")
@

As we can see in Figure \ref{fig:evoapps.uniform.mutation}, we can obtain much better fitness values, and even, in some cases, better energy expenses for the same fitness level. We represent in Figure \ref{fig:evoapps.energy.compare} a violin plot for both implementations, showing that for dimension 3 energy consumption is slightly better, as well as less dispersed, while for dimension 5 there is little difference.
%
...
@
%
However, let us look at the fitness values obtained in Figure \ref{fig:evoapps.fitness.compare}.
%
