...
ggplot(evoapps_results, aes(x=delta_PKG,y=delta_seconds,fill=color)) + geom_point(size=evoapps_results$point_size,shape=evoapps_results$shape) + geom_smooth(method="lm", formula=y~x, se=FALSE)+theme(legend.position="none") + labs(x="Energy (J)", y="Time (s)")
@
