...
for (dim in c(3,5)) {
  ...
  p1 <- ggplot(dim_results, aes(x=work, y=PKG, group=population_size)) + geom_violin() + theme_minimal() + labs(x="Population size", y="Energy (J)", title=paste("Energy distribution for dimension", dim)) + theme(legend.position="none")
  print(p1)
...
ggplot(all_results, aes(x=PKG,y=log_diff,color=dimension,shape=population_size)) + geom_point(alpha=0.5,size=all_results$point_size) + theme_minimal() + labs(x="Energy (J)", y="log(diff. target)")+
  scale_x_log10()
@
