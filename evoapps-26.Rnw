...
baseline_results %>% group_by(dimension,population_size) %>%
  summarise(median_energy=median(PKG), sd_energy=sd(PKG),
            median_time=median(seconds), sd_time=sd(seconds)) -> summary_baseline_results
