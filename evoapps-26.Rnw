...
This means that teh $\alpha$ percentage is the main one governing this exploration/exploitation balance. Please check table \ref{tab:params} for how the explorative/exploitative castes are organized depeding on the values used.
...
library(dplyr)
baseline_results <- read.csv("data/evoapps-1.11.7-baseline-bna-baseline-16-Oct-11-08-20.csv")
...
baseline_results %>% group_by(dimension,population_size) %>%
  summarise(mean_energy=mean(PKG), sd_energy=sd(PKG),
            mean_time=mean(seconds), sd_time=sd(seconds)) -> summary_baseline_results
...
for (dim in c(3,5)) {
  for ( pop_size in c(200,400)) {
    ...
    evoapps_results[ evoapps_results$dimension==dim & evoapps_results$population_size==pop_size,]$delta_PKG <-
      ...
      rep(summary_baseline_results[ summary_baseline_results$population_size == pop_size & summary_baseline_results$dimension==dim, ]$mean_energy,number_of_rows)
    ...
    evoapps_results[ evoapps_results$dimension==dim & evoapps_results$population_size==pop_size,]$delta_seconds <-
      ...
      rep(summary_baseline_results[ summary_baseline_results$population_size == pop_size & summary_baseline_results$dimension==dim, ]$mean_time,number_of_rows)
  }
}
...
evoapps_results %>% group_by(dimension,population_size,max_gens,alpha) %>%
  summarise(mean_energy=mean(delta_PKG), sd_energy=sd(delta_PKG),
            mean_time=mean(delta_seconds), sd_time=sd(delta_seconds)) -> summary_evoapps_results

# plot energy vs. evaluations with linear model walcom_lm
library(ggplot2)
ggplot(baseline_results, aes(x=evaluations,y=seconds)) + geom_point() + geom_smooth(method="lm", formula=y~x, se=FALSE)

PKG_0_evaluations <- unname(walcom_lm$coefficients[1])

baseline_results$diff_PKG <- baseline_results$PKG - PKG_0_evaluations

evoapps_results <- read.csv("data/sphere-evoapps-fix-bna-13-Oct-18-54-04.csv")
evoapps_results$work <- "BNA-mutation-perf-boost"

all_results <- rbind(baseline_results, evoapps_results)
