...
Concerns regarding the energy footprint of computing infrastructure are well-established \cite{sinha2001jouletrack,kurp2008green,lewis2008run}. However, the urgency of these concerns has intensified with the proliferation of resource-intensive metaheuristics \cite{irace,novoa2021measuring}, deep learning, and large language models \cite{10549890,rodriguez2024evaluatingenergyconsumptionmachine}. Consequently, the focus has shifted toward {\em greening} algorithm implementations. This involves an experimental framework where configurations are compared by measuring their energy consumption in specific environments and applying statistical techniques to identify the most efficient parameters for a given workload.

These stages—measurement and statistical comparison—are critical but fraught with technical challenges. The primary obstacle is that, unlike traditional performance metrics, it is impossible to isolate the energy consumption of a single process running on a specific core under a modern operating system (OS). Since hardware sensors and counters operate system-wide, researchers must synchronize system-wide measurements with process execution and subsequently attempt to isolate the system overhead. Standard practice involves taking baseline measurements to account for OS and framework overhead, then subtracting these from workload measurements to estimate the "real" energy consumed.

However, a second, more determinant challenge exists: the operational context. Processor states and operating conditions, particularly temperature, influence energy consumption in a non-linear fashion. This affects not only the absolute baseline but also the relative differences between the baseline and the active workload \cite{freina2024survey,cruz2025}. Modern chipsets and OSs actively manage thermal envelopes through Dynamic Voltage and Frequency Scaling (DVFS) and Dynamic Power Management (DPM) \cite{snowdon2005power}. For scientists working on non-dedicated personal computers, the system state is in constant flux. Power reduction measures and thermal throttling are triggered not just by the workload under study, but by background processes and OS core election policies, which may move tasks between "performance" and "efficiency" cores unpredictably.

One could attempt to mitigate this by strictly controlling the system state, but projecting such results to "real-world" systems is problematic. The interaction between system state and workload is often unknown; a configuration that appears optimal in a cooled, idle state may perform poorly when interacting with a thermally saturated system or a different core allocation.

This necessitates a new methodological approach: one that explicitly models the relationship between CPU package temperature, core election, and energy measurements. The goal of this paper is to propose and test methodologies that minimize the influence of these environmental variables. Through a series of extensive experiments, we analyze how temperature, as captured by internal sensors, interacts with workload implementation. We evaluate different experimental setups to identify a model capable of isolating the variance caused by algorithm parametrization from the variance caused by the underlying hardware state, ultimately providing a more robust framework for energy-aware algorithm design.
...
This study investigates the heuristic measurement of temperature's influence on energy consumption, an inquiry stemming from research into energy optimization for metaheuristics. Our workload in this paper is an implementation of the so-called {\em Brave New Algorithm (BNA)} \cite{bna}, a population-based metaheuristic written in Julia (version 1.11.9), utilizing its JIT-compiled architecture. To isolate thermal variables, all experiments are executed as single-threaded processes.
...
To mitigate environmental noise and baseline energy fluctuations, we employ an interleaved **"sandwich" protocol**. For every combination of $D$ and $N$, we perform two complete sequences (one for each value of $G$).

Each sequence consists of **30 workload repetitions** ($W$) interleaved with **31 baseline repetitions** ($B$), following the pattern: $B_1, W_1, B_2, W_2, \dots, W_{30}, B_{31}$.

* **Workload ($W$):** A full execution of the BNA.
* **Baseline ($B$):** Only the population generation phase, using the specified dimension and size.
...
Measurements were conducted on a desktop system with an AMD Ryzen 9 9950X 16-Core, 2-die processor. This CPU features a two-die architecture, allowing us to monitor temperature via two distinct sensors ({\tt Tccd1} and {\tt Tccd2}) using the {\tt k10temp-pci-00c3} driver. The operating environment is Ubuntu (Kernel {\tt 6.17.0-14-generic}. We utilize {\sf pinpoint} to tap the RAPL API \cite{rapl,khan2018rapl}. On this AMD architecture, measurements are restricted to Package (PKG) energy. A Perl script (running in superuser mode) automates the execution, processes readings from the `sensors` command, and outputs a CSV file.

All source code and experimental data are hosted in a public GitHub repository: \url{https://github.com/CeciMerelo/BraveNewAlgorithm.jl}.

<<icsme.base, echo=FALSE, results='hide'>>=
