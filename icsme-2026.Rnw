...
This begs for a experimental protocol that acts on temperatures, instead of doing it directly on running time via core assignment. Since it is impossible to assess {\em a priori} what will be the effect, we will have to test and model different configurations.
