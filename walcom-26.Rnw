...
<<walcom.evals, echo=FALSE, message=F, warning=F, fig.height=5, fig.cap="Energy consumption as a function of evaluations for all experiments. Point size is related to the alpha caste percentage. Lower is better in the y axis.">>=
summary(lm( PKG ~ evaluations * generations, data=walcom_results))
