...
%
\titlerunning{Energy consumptio of exploration/exploitation balance}
...
<<walcom.results, echo=FALSE, results='asis', fig.cap="Energy as a function of time for all experiments">>=
...
walcom_results$type <- as.factor(walcom_results$type)
walcom_results$size <- as.factor(walcom_results$size)
ggplot(walcom_results, aes(x=seconds,y=PKG,color=type,shape=size)) + geom_point() + theme_minimal() + labs(x="Time (s)", y="Energy (J)")

@


<<walcom.models, echo=FALSE, results='asis'>>=
evaluations_model <- glm(evaluations ~ type + size + max_gens + alpha, data=walcom_results)

walcom_results %>% group_by(type,size,max_gens,alpha) %>%
  summarise(mean_energy=mean(PKG), sd_energy=sd(PKG),
            ...
            mean_diff_fitness=mean(diff_fitness), sd_diff_fitness=sd(diff_fitness)) -> summary_results
@
...
\bibliography{ours,ga-energy,GAs}
