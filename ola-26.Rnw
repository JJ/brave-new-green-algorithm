...
ola_mixed_v11_8$accumulated_time <- cumsum(ola_mixed_v11_8$seconds)

ggplot( ola_mixed_v11_8, aes(x=accumulated_time, y=PKG, color=as.factor(work)), group=as.factor(work) ) +
...
In Figure \ref{fig:ola.mixed} we have mixed baseline and workload runs making the workload run right after the baseline run; instead of averaging baseline runs and then subtracting it from the individual workload runs, we subtract the baseline run that has been made before the workload run from it.
