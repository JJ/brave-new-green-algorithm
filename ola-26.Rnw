...
<<ola.deltas, echo=FALSE, warning=F, message=F,fig.cap="Workload-exclusive (delta) energy spent vs. delta  for the initial configuration vs. measurements with sampling period = 100 ms", fig.height=3>>=
...
<<ola.micro.plot, echo=FALSE, warning=F, message=F,fig.cap="Boxplot comparing different implementation of genetic operators for different problem (Dim) and population sizes.",fig.height=4, fig.pos="h!tb">>=
...
<<ola.micro.hysteresis, echo=FALSE, warning=F, message=F,fig.cap="PKG energy data vs. accumulated time for the two baseline runs we are examining. Baseline runs should have a very small variation; this plot of an example shows that is not the case.", fig.pos="h!tb", fig.height=3>>=
...
<<ola.mixed, echo=FALSE, warning=F, message=F,fig.cap="Energy spent and delta energy (workload - baseline) in an interleaved workload (ola-mixed) and baseline (base-ola-mixed) chart in a specific run.",fig.height=3, fig.pos="h!tb">>=
...
<<ola.mixed.comparison, echo=FALSE, warning=F, message=F,fig.cap="Comparing separated  vs. interleaved  layout of baseline and workload runs; workload-exclusive energy (Delta PKG) is compared for different problem sizes, as well as population (Pop. size) and maximum number of generations without improving fitness.",fig.height=3, fig.pos="h!tb", fig.height=4>>=
...
mixed_vs_separated_workload %>%
  mutate(work = factor(work,
                       levels = c("ola-mixed", "Workload v2"), # Valores originales en tu dataset
                       labels = c("Interleaved", "Separate"))) -> mixed_vs_separated_workload
...
ggplot(mixed_vs_separated_workload, aes(x=population_dimension, y=delta_PKG, color=as.factor(work))) +
  ...
  xlab("Version") +
  ylab("Delta Package energy (Joules)") +
  labs(color="Implementation version",labels=c("Interleaved","Separate")) + # x ticks rotated
