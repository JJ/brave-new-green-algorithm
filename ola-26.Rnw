...
<<ola.mixed.comparison, echo=FALSE, warning=F, message=F,fig.cap="Workload energy spent compared with different ways of measuring the workload effect; we also compare for two values of the maximum number of generations run without improving fitness.",fig.height=4, fig.pos="h!tb", fig.height=4>>=
...
<<ola.v11_8, echo=F, warning=F, message=F, fig.cap="Comparing Julia versions", fig.height=4, fig.pos="h!tb">>=
...
This paper, whose development can be also checked at \url{https://github.com/CeciMerelo/BraveNewalgorithm.jl}, attempts to propose a new methodology in measuring energy looking to optimize the implementation of population-based evolutionary algorithms. We have found that decreasing the sampling rate reduces overhead and improves precision; repeating runs where errors occur so that a fixed number of runs is reached improves reproducibility.
