...
lion_model_data <- read.csv("data/lion-1.11.8-bna-fix-rand.csv") %>%
  mutate(
    ...
    evaluations = as.numeric(evaluations),
    max_gens = factor(max_gens),
    alpha = factor(alpha)
  )

parameter_levels <- data.frame(
  parameter = c("dimension", "population_size", "max_gens", "alpha"),
  levels_in_dataset = c(
    dplyr::n_distinct(lion_model_data$dimension),
    dplyr::n_distinct(lion_model_data$population_size),
    dplyr::n_distinct(lion_model_data$max_gens),
    dplyr::n_distinct(lion_model_data$alpha)
  )
)

knitr::kable(
  parameter_levels,
  col.names = c("Parameter", "Observed levels in data"),
  caption = "Observed support of key algorithm parameters in the final LION dataset."
)
```

In this specific file, `alpha` appears with a single observed value, so it has no within-dataset variance for coefficient estimation; `max_gens` appears with two levels and is reflected through realized `generations` and `evaluations` in the multi-stage modeling path above.

```{r lion-expanded-time-model}
time_model <- lm(
