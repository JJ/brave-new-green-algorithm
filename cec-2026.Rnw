...
<<cec.baseline, echo=FALSE, fig.cap="PKG energy measured vs. time for a baseline measurement.", fig.height=2.5>>=
...
library(ggplot2)
ggplot(baseline_ola_100s, aes(x = cum_seconds, y = PKG)) +
    geom_line() +
    labs(
...
What is observed in \ref{fig:cec.baseline} is that there are different levels of background energy consumption; up to 2000 seconds it is substantially the same, but then it changes to a higher level (totally unrelated to an actual change in parametrization) and then eventually to a slightly higher one, around 400 Joules.
