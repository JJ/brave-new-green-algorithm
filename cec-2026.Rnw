...
%
\begin{abstract}
As we proceed in the incorporation of energy efficiency in the design of our algorithms, establishing a solid methodology for energy profiling and eventual comparison of algorithms or implementations is essential for a solid foundation of any further work. The main issues to overcome is the lack of specific per-process measurement of energy consumption but, above that, the fact that we are going to be measuring a system that is actively trying to optimize energy consumption itself, and doing so for the whole system, above and beyond the workload we are trying to measure. In this paper we will be proposing a two-stage methodology for energy profiling in population-based metaheuristics. The first stage will consist in an experimental design, that is, how to run a series of experiments to measure the energy spent by a specific configuration and implementation of the algorithm. The second phase will consist on the statistical processing of these results to obtain a measure of energy consumption with a certain degree of certainty. Finally, we will be applying these measurements to an type of evolutionary algorithm, called Brave New Algorithm, written in the language Julia, to find out the impact on energy consumption of changes at different levels.
