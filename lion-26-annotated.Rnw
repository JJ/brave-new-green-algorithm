...
\important{Three core findings:
(1) Baseline energy is significant and noisy;
(2) `max\_gens=10` is an energy-lean default;
(3) Energy-fitness is a policy trade-off, not a single objective.}
\reference{JJ Merelo et al., {\em Is there a (carbon-) free lunch? Energy/performance tradeoffs in population-based metaheuristics}, LION 20, 2026.}
\codelink{Interactive summaries: \href{lion-26-explainer.html}{LION Explainer} and \href{lion-ola-progression.html}{Research Progression (LION to OLA)}.}
...
But for any kind of change, including the balance of operators used, it is extremely complicated or impossible to know in advance how the energy profile might change:
...
To carry out these experiments, we have used a methodology like the one used previously to compare different parametrizations, implementations and versions \cite{low-level}:
...
\important{Software platform drift: Upgrading Julia from 1.11.7 to 1.11.8 appeared to increase energy in initial LION tests, but OLA confirmed it actually saves ~10\% once thermal confounds are fixed.}
...
\methodstep{Baseline defined as population generation only. 20\% trimmed mean used to summarize baseline PKG across 240 runs.}
...
\keyfinding{Baseline PKG energy is highly variable. The multi-modal clusters in Fig. 1 reflect the hardware cycling through different thermal operating states during the session.}
...
We should take into account that profiling for every kind of algorithm needs its own methodology that is adequate to the dispersion in the measurements taken; however, a high dispersion combined with time (or temperature) related effects will always lead to a certain amount of uncertainty in the conclusions. That will not invalidate these conclusions, but they will have to be explained and factored in when any kind of result is reached \cite{merelo25:time}.

After performing experiments running just the baseline, the results obtained after performing measurements with the actual workload will be examined next.
...
\keyfinding{In Fig. 3 (left), population size (triangles vs circles) has less impact on workload energy than the dimension (color) or stopping criterion (size).}
...
\keyfinding{Mutation micro-optimization (reducing allocations) consumed slightly more energy per evaluation but improved exploration, shifting the fitness frontier.}
...
\keyfinding{Switching to a uniform random generator for mutation allowed reaching much better fitness levels for a comparable energy budget.}
...
\keyfinding{Exploration/exploitation balance: Devoting less of the population to exploitation (lower alpha) resulted in both better fitness and lower energy profile.}
...
\important{Final takeaway on Julia 1.11.8: Initial LION results suggested 1.11.8 was less efficient. Later OLA research revealed this was an artifact of cross-session thermal drift. Use 1.11.8.}
