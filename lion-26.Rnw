...
% El texto previo que explica el balance entre exploración y explotación y el
% costo computacional de los operadores es interesante y se debería expander un poco.
% Por ejemplo:
% - En algunos casos los operadores no generan nuevos individuos (cuando se
% utilizan estructuras mutables).
% - Hay casos en los que borrar, acutalizar y agregar objetos en memoria tiene
% distinto costo.
% - Se podría agregar el otro balance entre el costo de ejecución de las
% instrucciones y la mejora en la búsqueda que generan, que a su vez podría reducir el
% costo de ejecución total.
% - Mario
% He intentado reescribirlo. Si se te ocurre algo más - JJ
...
% Podría mencionarse la complejidad adicional que agregan aspectos de bajo nivel como la recolección de basura,
% sobre todo si se realizan asignaciones frecuentes de memoria:
% https://docs.julialang.org/en/v1/devdocs/gc/

% Sería conveniente aclarar si estos efectos se consideran explícitamente en este
% trabajo o si se asumen como parte del costo de ejecución interno.
% - Mario
% Son parte del coste de ejecución, en principio, aunque la carga de trabajo debe minimizar también los efectos de GC. Puedes incluir algo, pero ya te digo que no hay mucho espacio, solo hay 15 páginas - JJ
