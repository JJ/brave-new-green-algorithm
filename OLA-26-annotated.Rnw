...
% ============================================================
%  ANNOTATION SETUP  (added for OLA-26-annotated.Rnw only)
% ============================================================

% Widen the right margin so todonotes has room for side notes.
% The LNCS class fixes \textwidth / \textheight; geometry overrides them.
% Larger right margin (6.5cm) gives wider note boxes -> fewer lines per note.
% Larger bottom margin (3.5cm) keeps text higher so stacked notes don't overflow.
\usepackage[a4paper,
            left=2.5cm, right=6.5cm,
            top=2.5cm,  bottom=3.5cm,
            marginparwidth=5.5cm, marginparsep=4mm]{geometry}
...
\important{Three core contributions:
(1) raise RAPL sampling period to 100\,ms;
(2) interleave baseline and workload runs to make operating conditions closer;
(3) filter zero-energy readings for reproducibility.}
\reference{JJ Merelo, Cecilia Merelo Molina {\em Best practices in measuring energy
consumption in population-based metaheuristics}, in Proceedings OLA'26
International Conference on Optimization and Learning, pp 183-194.}
...
There are several reasons why this occurs, but there are two main ones: first, modern computer systems proactively manage energy consumption, so the interaction with user-level measures is not straightforward; second, the tooling available makes impossible to separate the actual workload from the overhead created not only by the system, but also by the runtime system of the language we are using, be it compiled (including just-in-time compiled, or compiled to bytecode) or interpreted \cite{en12112204}. Using baseline measurements which are then averaged causes issues such as negative workload measured energy \cite{low-level,lion24}.\important{Negative delta-PKG values arise when a shared baseline average is subtracted from individual workload runs whose system state differs. Interleaving (Sect.\,4) is the fix.} At the same time, hysteretic effects \cite{cotta25} make it difficult to reproduce results, since the physical conditions of the hardware platform will affect measurements in a non-deterministic way. All this is combined with the specific issues with the measurement tool we are using and its possible lack of precision or other kinds of errors.
...
\codelink{Julia package: \url{https://github.com/cecimerelo/BraveNewAlgorithm.jl}. All experimental CSV files in \texttt{data/} are produced by running scripts in this repo. {\em Chunk} refers to code fragments embedded in the \href{https://github.com/JJ/brave-new-green-algorithm/blob/main/OLA-26-annotated.Rnw}{source code of this paper}}
...
\methodstep{Sampling period raised from 50\,ms (default) to 100\,ms to reduce RAPL measurement overhead. Compare two baselines: \texttt{evoapps-1.11.7-baseline} (50\,ms) vs.\ \texttt{ola-base} (100\,ms).}
...
kable(initial_baseline_table, "latex",
      col.names = c("Population size","Dimension","Baseline PKG (J)","Sampling = 100ms", "Significant"),
      caption = "Baseline energy consumption for different sampling frequencies",
      escape=FALSE) %>%
  kable_styling( full_width = F)

@
\codelink{Chunk \texttt{ola.base.frequency}: reads \texttt{data/evoapps-1.11.7-baseline-*.csv} (50\,ms) and \texttt{data/ola-base-*.csv} (100\,ms), runs Wilcoxon tests, and renders Table\,\ref{tab:ola.base.frequency}.
\vspace{2pt}
\texttt{wilcox.test(work1\_data, work2\_data)}
\vspace{2pt}
Significance marker \texttt{+}/\texttt{-} via \texttt{p\_value < 0.05}.}
...
\keyfinding{Changing the RAPL sampling period alone (50\,ms $\to$ 100\,ms) produces statistically significant differences in the baseline PKG readings (Wilcoxon, $p{<}0.05$), confirming that measurement configuration is itself an experimental variable.}
...
@
\codelink{Chunk \texttt{ola.deltas}: defines helper \texttt{compute\_deltas()} that subtracts the per-\{dimension, pop\_size\} median baseline PKG from each workload row.
\vspace{2pt}
Key line: \texttt{delta\_PKG <- workload\$PKG - rep(baseline\_summary\$median\_energy, n)}
\vspace{2pt}
Data: \texttt{evoapps-1.11.7-fix-rand-*.csv} and \texttt{ola-1.11.7-ola-*.csv}.}
...
\keyfinding{The scatter plot (Fig.\,\ref{fig:ola.deltas}) shows that the 100\,ms configuration tightens the delta-PKG distribution and shifts it upward, confirming reduced measurement overhead — yet unrealistic sub-50\,J readings remain, indicating further steps are needed.}
...
kable(initial_vs_v2_baseline_table, "latex",
  ...
  kable_styling( full_width = F)
@
\codelink{Chunk \texttt{ola.micro}: compares baseline PKG between v1 (\texttt{ola-base-*.csv}) and memory-optimised v2 (\texttt{ola-1.11.7-v2-baseline-*.csv}) implementations.
\vspace{2pt}
Same \texttt{wilcoxon\_tests()} helper reused here.}
...
\keyfinding{Micro-optimisation of individual/population generation (fewer memory allocations) yields mixed baseline results: some improvements, some regressions, depending on dimension and population size. Workload comparison is the decisive test.}
...
@
\codelink{Chunk \texttt{ola.micro.plot}: applies \texttt{compute\_deltas()} to workload data from \texttt{ola-1.11.7-v2-ola-v2-*.csv} and generates violin plots (Fig.\,\ref{fig:ola.micro.plot}).}
...
@
\codelink{Chunk \texttt{ola.micro.hysteresis}: adds \texttt{cumulative\_time <- cumsum(seconds)} column to each dataset and plots PKG vs.\ elapsed time to reveal the thermal regime change around 2000\,s.}
...
\keyfinding{Plotting PKG vs.\ cumulative time (Fig.\,\ref{fig:ola.micro.hysteresis}) reveals a clear thermal regime shift at ${\approx}2000$\,s. This hysteretic drift partially explains the contradictory micro-optimisation results, and motivates \emph{interleaved} baseline–workload measurement.}
...
@
\codelink{Chunk \texttt{ola.mixed}: uses \texttt{data/ola-1.11.7-mixed-*.csv} where baseline and workload rows are interleaved. Delta is computed row-by-row:
\vspace{2pt}
\texttt{delta\_PKG[i] <- PKG[i] - PKG[i-1]}
\vspace{2pt}
rather than subtracting a global mean, ensuring both measurements share the same thermal state.}
...
\methodstep{Interleaved measurement: each workload run is immediately preceded by a baseline run. Delta-PKG is the pairwise difference rather than workload minus global mean. This is the core methodological innovation of the paper.}
...
#   facet_wrap( ~ work )
@
\codelink{Chunk \texttt{ola.mixed.comparison}: compares separated (\texttt{ola-workload-v2}) vs.\ interleaved (\texttt{ola-mixed}) delta-PKG distributions, also broken down by \texttt{max\_gens} to show the algorithm-parameter dimension.}
...
\keyfinding{Interleaved measurement (Fig.\,\ref{fig:ola.mixed.comparison}) reduces variance and eliminates most negative delta-PKG values compared with the separated-baseline approach, validating the pairwise subtraction strategy.}
...
ggplot(ola_v11_7_vs_8_data, aes(x=population_dimension, y=delta_PKG, color=as.factor(work))) +
  ...
  facet_wrap(~ max_gens)
@
\codelink{Chunk \texttt{ola.v11\_8}: reads \texttt{data/ola-1.11.8-mixed-inverted-*.csv} (Julia v1.11.8) and compares delta-PKG against the v1.11.7 interleaved dataset via notched box-plots. y-axis clipped to [0,\,250].}
...
\keyfinding{Julia v1.11.8 consistently uses less energy than v1.11.7 across all population/dimension combinations (Fig.\,\ref{fig:ola.v11_8}), most noticeably for \texttt{max\_gens\,=\,25}. Always test with the latest patch release of your runtime.}
...
ggplot(ola_0_vs_no0_data, aes(x=population_dimension, y=delta_PKG, color=as.factor(work))) +
  ...
  facet_wrap(~ max_gens)
@
\codelink{Chunk \texttt{ola.no0}: loads \texttt{data/ola-1.11.8-ola-no0-*.csv} where zero-PKG runs are automatically retried. Compares dispersion against the version that allows zero readings via notched box-plots.}
...
\keyfinding{Filtering out zero-PKG readings (Fig.\,\ref{fig:ola.no0}) reduces dispersion and improves reproducibility. The \emph{dry-run} warm-up effect is a beneficial side effect: it stabilises the hardware thermal state before the measurement proper begins.}
...
\important{Summary of all three methodology steps and their impact:
(1) 100\,ms sampling $\to$ reduced RAPL overhead;
(2) pairwise interleaving $\to$ eliminates negative deltas;
(3) zero-run retry $\to$ lower variance and warm-up stabilisation.}
...
\keyfinding{Final result: ${\approx}10\%$ energy improvement from memory-allocation micro-optimisations and ${\approx}10\%$ from upgrading Julia v1.11.7 $\to$ v1.11.8 — detectable only because the three methodology improvements raised measurement precision sufficiently.}
