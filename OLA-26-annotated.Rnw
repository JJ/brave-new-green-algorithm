...
\usepackage[a4paper,
            left=2cm, right=5.5cm,
            top=2.5cm,  bottom=2.5cm,
            marginparwidth=4.5cm, marginparsep=4mm]{geometry}

% Main annotation package
\usepackage[colorinlistoftodos, textwidth=4.5cm, shadow]{todonotes}

% Listings for syntax-highlighted R code inside annotations
\usepackage{listings}
\lstset{
  language=R,
...
\newcommand{\keyfinding}[1]{%
  \todo[color=clrKey, size=\scriptsize]{\textbf{Key finding:} #1}}
\newcommand{\codelink}[1]{%
  \todo[color=clrCode, size=\scriptsize]{\textbf{Code:} #1}}
\newcommand{\methodstep}[1]{%
  \todo[color=clrMethod, size=\scriptsize]{\textbf{Methodology:} #1}}
\newcommand{\important}[1]{%
  \todo[color=clrImportant, size=\scriptsize]{\textbf{Note:} #1}}
% ============================================================

\begin{document}

% ---- Overview of all annotations ----
\listoftodos[Annotation index]
\newpage
% -------------------------------------
...
\maketitle % \begin{abstract} Although developing energy-aware software has been
...
(2) interleave baseline and workload runs to eliminate hysteresis;
...
\codelink{Julia package: \url{https://github.com/cecimerelo/BraveNewAlgorithm.jl}. All experimental CSV files in \texttt{data/} are produced by running this package.}
...
\vspace{2pt}
\lstinline[language=R]{wilcox.test(work1_data, work2_data)}

\vspace{2pt}
Significance marker \texttt{+}/\texttt{-} via \lstinline[language=R]{p_value < 0.05}.}
...
@
\codelink{Chunk \texttt{ola.deltas}: defines helper \lstinline[language=R]{compute_deltas()} that subtracts the per-\{dimension, pop\_size\} median baseline PKG from each workload row.

\vspace{2pt}
Key line: \lstinline[language=R]{delta_PKG <- workload$PKG - rep(baseline_summary$median_energy, n)}
...
\vspace{2pt}
Same \lstinline[language=R]{wilcoxon_tests()} helper reused here.}
...
@
\codelink{Chunk \texttt{ola.micro.plot}: applies \lstinline[language=R]{compute_deltas()} to workload data from \texttt{ola-1.11.7-v2-ola-v2-*.csv} and generates violin plots (Fig.\,\ref{fig:ola.micro.plot}).}
...
@
\codelink{Chunk \texttt{ola.micro.hysteresis}: adds \lstinline[language=R]{cumulative_time <- cumsum(seconds)} column to each dataset and plots PKG vs.\ elapsed time to reveal the thermal regime change around 2000\,s.}
...
\vspace{2pt}
\lstinline[language=R]{delta_PKG[i] <- PKG[i] - PKG[i-1]}
